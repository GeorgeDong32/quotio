...
nonisolated enum AIProvider: String, CaseIterable, Codable, Identifiable, Sendable {
    ...
    var supportsQuotaOnlyMode: Bool {
        switch self {
        case .claude, .codex, .cursor, .factoryDroid, .gemini, .antigravity, .copilot, .devin, .grok, .openRouter, .trae, .glm, .warp, .kiro:
            return true
    ...
    var supportsManualAuth: Bool {
        switch self {
        case .cursor, .trae, .devin, .grok, .glm:
    ...
    var usesAPIKeyAuth: Bool {
        switch self {
        case .glm, .warp, .factoryDroid, .openRouter:
            return true
        default:
            return false
        }
